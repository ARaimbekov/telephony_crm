...
from leads.models import Number, Atc, Lead, Company, Apparats, User


def migrate_numbers(file):
    """
    Миграция номеров из файла
    Формат csv файла:
    k;6501;0004f25e8dd9;Polycom;1;6501@qwe;10.102.27.250;ИНМ СПО
    """
        ...
        for line in f:
            try:
                ...
                if line[1] and line[6]:
                    ...
                    #atc = Atc.objects.filter(pk=12).first()
...
def migrate_mac(file):
    """
    Миграция учетных записей для телефонных аппаратов из файла
    Формат csv файла:
    k;6501;0004f25e8dd9;Polycom;1;6501@qwe;10.102.27.250;ИНМ СПО
    """

    company = Company.objects.filter(name='OOO "ИНК"').first()  # При необходимости сменить компанию
    user = 'system'
    with open(file, 'r', encoding='utf-8') as f:
        for line in f:
            try:
                ...
                if len(line) > 0:
                    ...
                    atc = Atc.objects.filter(ip_address=atc_ip).first()
                    # atc = Atc.objects.filter(pk=21).first()
